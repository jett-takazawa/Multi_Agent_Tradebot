...
from Data_Collect import save_signal, save_reccs, save_exec
...
def call_vertex_model(model_id: str, system_prompt: str, user_prompt: str) -> str:
    ...
    Args:
        model_id (str): The ID of the fine-tuned model to use.
        system_prompt (str): The system instructions for the model.
        user_prompt (str): The user-provided prompt (e.g., candle data).
...
def exec_de(
    ...
    # 4) Call the model
    respondos = client.chat.completions.create(
        model="o4-mini-2025-04-16",
        seed=69,
        reasoning_effort="high",
        messages=[
            {"role":"system","content":SYSTEM_PROMPT_EXEC_DECISION},
...
def run_sys(symbol: str, vol_score: float):
    ...
    if decision == "reevaluate":
        ...
        print("here we are")
        reevaluated = client.chat.completions.create(
            model="o4-mini-2025-04-16",
            reasoning_effort="high",
            seed=69,
            messages=[
                {"role": "system", "content": full_system_reval},
                {"role": "user",
                 "content": build_user_msg(candles, symbol) + f"\n\n{hint}"}
            ]
...
def find_volatility(
    ...
    *,
    ...
    """Return a trade decision string based on news‑driven volatility.

    Parameters
    ----------
    ticker : str
        Stock symbol to evaluate.
    volatility_threshold : float, default 0.7
        Decision cut‑off (0‑1). Score ≥ threshold ⇒ NO_TRADE.
    risk : float | None, optional
        Convenience alias for *volatility_threshold* (if supplied, overrides it).
    verbose : bool, default False
        Print fetched summaries count and GPT score for debugging.
    ...
    -------
    str
        "NO_TRADE (Volatility)" or "TRADE_OK".
    """
    #fetch_recent_cand(ticker, RESOLUTION_MIN, BARS_PER_PROMPT, True)

    # 1) Fetch news summaries -------------------------------------------------
    global DATA_DICT
    global DICTupdate
    try:
        ...
        if verbose:
            print("[VOL] Fetch error – failing safe → NO_TRADE")
        score = 1
    ...
    if not summaries:
        # No headlines – assume calm (override via threshold=0 if you want to block)
        score = 0
    ...
    completion = client.chat.completions.create(
        model="o4-mini-2025-04-16",          # current snapshot ID
        reasoning_effort="low",             # <-- magic switch, #High intelligence, supports fine tuning, and images
        seed = 69,
        temperature= 1,
    ...
    print(score)
    #except Exception:
       # score = 1.0  # safest default
        #print("Im trolling")

    if verbose:
        print(f"[VOL] {ticker} score = {score:.3f}, threshold = {volatility_threshold}")
    
    DATA_DICT["News_Volatility"] = score
    # 4) Decision -------------------------------------------------------------
    if score >= volatility_threshold:
        # ---------- fresh, local row dict --------------------------------
        row = {c: None for c in FIELDNAMES}             # ← changed
        row.update({                                    # ← changed
...
for x in lists:
    if find_volatility(x, risk=0.75, verbose=True) == "TRADE_OK":
       run_bot(str(x))
    else: 
        print(find_volatility(x, risk=0.7, verbose=True))
        """